    ...
    //log tt + ", " + av:z + ", " +ra:z+", "+ta:z+", "+(-SHIP:CONTROL:ROLL) to "stat.csv".
        ...
        if tt > sac_ct {
            ...
            if tune {
                ...
                sac_tune(sac_yaw_arr, tt, -sac_x0:yaw, sac_v0:y, ff:yaw-fo:yaw, av:y, sac_tar_sp:y, SHIP:CONTROL:YAW, true).
...
function sac_speed_limit2 {
    ...
    parameter v0.
    return v(tv:x,
            sac_speed_limit_x2(-nt:yaw, tv:y, sp:y, sac_yaw_arr, v0:y),
            tv:z).
...
function sac_speed_limit_x2 {
    ...
    local s to sign(x).
    local lv to -s*sqrt(2*x*sac_max_a_x(arr, s, v0)/4). // TODO
    //local lv to -(x*sac_w^2+sac_max_a_x(arr, s, v0)/2)/2/sac_w.
...
function sac_tune {
    ...
    if state > 0 {
        ...
        local df to x0 * tt * e0 - tt * xt.
        local ta to calcTA(x0, v0).
        local f to (x - xt) * abs(ta) / ac /sac_w.
        ...
        local dac to 0.
        if f > df set dac to 1.
        else if f < -0.5*df set dac to -0.5.
        else set dac to f/df.
        ...
        if c0<>c {
            set ac to ac + ac * dac.
            set za to (za - ta)*(1+dac) + ta.
        }

        //if c0<>c or ((c < 1 or dza > 0) and (c > -1 and dza < 0)) {
            set za to za + dza.
        //}

...
function sac_tune {
    ...
    set sac_stat_ar2 to sac_stat_ar2/sac_stat_n.

    sac_tune_x(sac_pitch_arr, mra:x, mta:x, sac_stat_ar0:x, sac_stat_at2:x, sac_stat_ar2:x, sac_pitch_debug).
    sac_tune_x(sac_yaw_arr, mra:y, mta:y, sac_stat_ar0:y, sac_stat_at2:y, sac_stat_ar2:y, sac_yaw_debug).
    sac_tune_x(sac_roll_arr, mra:z, mta:z, sac_stat_ar0:z, sac_stat_at2:z, sac_stat_ar2:z, sac_roll_debug).
...
function sac_tune_x {
    ...
    local acc to 1.
    if not sac_manual_ac {
        local dc to c1 - c0.

        local sta2 to at2 - mta^2.
        local sra2 to ar2 - mra^2. // It was S1/S2
        if (sra2<0) {
            log_log("WARN: sra2 = "+sra2).
            set sra2 to 0.
        }
        if (sta2<0) {
            log_log("WARN: sta2 = "+sta2).
            set sta2 to 0.
        }
        local sart to sqrt(sra2*sta2).

        local dck to dc*ac0.
        local acck to 1/(sta2 + dck^2 + 1). // TODO move 4 here somehow (probably +3*ac0), +1 is here just to avoid divide by zero issue
        local maacc to (sart-sta2)*acck/4 + 1.

        local dcacc to mra - ar0.
        set dcacc to (dcacc*dck-dck^2)*acck/4 + 1.

        set acc to maacc*dcacc.
    ...
    //if false {
        if debug {
            stat_log("tune",
                    {return list("mra", "mta", "sra", "sta", "ac").},
                    list(mra, mta, sqrt(sra2), sqrt(sta2), ac0)).
        }
    }
...
function sac_ac {
    ...
    if sac_manual_ac {
        set sac_pitch_arr[0] to ac_pitch.
        set sac_yaw_arr[0] to ac_yaw.
        set sac_roll_arr[0] to ac_roll.
    }
}

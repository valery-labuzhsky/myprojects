...
print R(13, 352, 268) * HEADING(90, 45).
print R(12, 351, 269) * HEADING(90, 45).
...
function init {
  parameter x.
  parameter deb.
  return list(0, normX(x, 0), 0, 0, time:seconds, 0, 20, 0, 0, 0, 0, deb, 0, 0, 0, 0).
}
...
function seek {
  ...
  local idc to arr[12].
  local dat to arr[13].
  local datt to arr[14].
  local idatt to arr[15].
  ...
  if t < t2 {
	...
	if deb
	{
	  print ship:facing:inverse.
	  print "x = " + x.
	  print "v2 = " + v2.
	  print "a2 = " + a2.
	  //print " ".
	}
	local dat2 to (a2 - a   - odc * acc) / dt.
	set datt to (dat2 - dat) / dt.
	set idatt to idatt * 0.9 + datt * dt.
	if deb
	{
	  //print "dat = " + dat2.
	  //print "datt = " + datt.
	...
	if state > 2 {
	  set idc to idc + odc.
	  local dacc to a2 - a - odc * acc - dat * dt.
	  //set ida to ida + dacc * idc.
	  set ida to ida + idatt * dt.
	  set acc to ida * 0.1 * idc + acc.
	  set ida to ida * 0.9.
	  if deb {
	    //print "dat = " + dat * dt.
	    //print "dacc = " + dacc.
	    //print "dc = " + (ida * 0.1 * idc).
	    //print "idc = " + idc.
	    print "ida = " + ida.
	  }
	...
	if state > 2 {
	  ...
	  set dc to (ta - fa)/acc.//(ta - za)/ acc - c.
	}
	set tc to c + dc.
	set a to a2.
      }
